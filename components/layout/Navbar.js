...
export default function Navbar() {
    ...
    return (
        <nav ref={navRef} className="fixed top-0 left-0 w-full h-16 bg-white text-black z-50 border-b border-gray-300">
            <div className="grid grid-cols-2 lg:grid-cols-3 items-center h-16 px-8">
                {/* Logo - Left */}
                <div className="flex justify-start">
                    <Link href="/" className="flex items-center text-3xl font-light !text-black hover:text-gray-600 transition-colors">
                        <img
                            src="/images/logo.png"
                            alt="logo"
                        ...
                        <div className="flex items-center  px-6 py-2 space-x-8">
                            ...
                            <div className="flex items-center space-x-8">
                                {menuGroups.map((group) => {
                                    if (group.groupName === "Daily Logs") {
                                        return (
                                            <Link
                                                key={group.groupName}
                                                href="/daily-logs"
                                                className={`text-sm font-light tracking-tight transition-colors duration-200 ${
                                                    isActive('/daily-logs')
                        ...
                        <>
                            ...
                            <div className="hidden md:flex items-center space-x-4">
                                <div className="text-right">
                                    <div className="text-sm font-light text-black">{session.user.name.toUpperCase()}</div>
                                    {session.user.roles && session.user.roles.length > 0 && (
                                        <div className="text-xs font-light text-gray-500">{session.user.roles.join(', ').toUpperCase()}</div>
                                    )}
                                </div>
                                {session.user.name && (
                                    <Avatar
                                ...
                                <button
                                    ...
                                    className="text-sm font-light text-black hover:text-gray-500 transition-colors border-b border-black hover:border-gray-500"
                                >
                                    LOGOUT
                                </button>
                            </div>
                ...
                <div className="lg:hidden bg-white border-t border-black">
                    <div className="px-8 py-6">
                        ...
                        <div className="py-6 space-y-4">
                            <Link
                                href="/daily-logs"
                                className={`block text-sm font-light tracking-tight transition-colors ${
                                    isActive('/daily-logs')
                            ...
                            {menuGroups.filter(group => group.groupName !== "Logs").flatMap(group => group.items).map((item) => (
                                <Link
                                    key={item.name}
                                    href={item.href}
                                    className={`block text-sm font-light tracking-tight transition-colors ${
