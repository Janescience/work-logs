...
import { 
  faUsers, faSpinner, faUserCheck, faEdit, faTrash, faSave, 
  faChevronDown, faChevronUp, faClock, faListCheck, faCalendarDays 
} from '@fortawesome/free-solid-svg-icons';
import MyJiras from '@/components/MyJiras';
import WorkLogCalendar from '@/components/WorkCalendar';
import { toast } from 'react-toastify';
import TeamSummary from '@/components/TeamSummary'; // *** 1. IMPORT COMPONENT ใหม่ ***
import TeamRetrospective from '@/components/TeamRetrospective'; // 1. IMPORT COMPONENT ใหม่

// ... (ส่วนของ getAvatarUrl, getWorkingDaysInMonth, calculateMonthlyHours, countActiveJiras ไม่เปลี่ยนแปลง) ...
  ...
  return (
    <div className="min-h-screen bg-white p-6">
      <div className="max-w-7xl mx-auto">
        ...
        {/* *** 2. เพิ่ม Section TeamSummary ที่นี่ *** */}
        <div className="mb-8">
        ...
        {!loadingData && Object.keys(teamData).length > 0 && (
          <div className="space-y-2">
            {Object.values(teamData).map((userData) => {
              const { memberInfo, jiras } = userData;
              if (!memberInfo) return null; // Add a guard clause
              const isCollapsed = collapsedMembers[memberInfo.userId] !== false; // Default to true
              
              return (
                <div key={memberInfo.userId} className="border border-gray-200 rounded-lg overflow-hidden">
                  <div 
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600">{memberInfo.email}</p>
                        </div>
                      </div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-md max-h-[80vh] overflow-y-auto">
              ...
              <div className="mb-4">
                ...
                <input
                  ...
                  className="w-full text-black px-3 py-2 border border-gray-300 rounded-md focus:ring-black focus:border-black"
              ...
              <div className="flex justify-end space-x-4">
                ...
                <button
                  ...
                  className="bg-black hover:bg-gray-800 text-white py-2 px-4 rounded flex items-center gap-2"
                  disabled={!teamName.trim() || selectedMembers.length === 0}
                >
                  {isEditMode ? 'Update Team' : 'Create Team'} ({selectedMembers.length})
                </button>
              </div>
            </div>
          </div>
